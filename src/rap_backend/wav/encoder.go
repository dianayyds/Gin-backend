...
func (e *Encoder) addBuffer(buf *audio.IntBuffer) error {
	...
	seelog.Debugf("addbuffer, frame :%d", frameCount)
...
func (e *Encoder) writeHeader() error {
	seelog.Debugf("writeHeader, %s", e.wroteHeader)
	...
	seelog.Debugf("writeHeader step00")
	...
	seelog.Debugf("writeHeader step0 written byte:%d", e.WrittenBytes)

	if e.WrittenBytes > 0 {
		return nil
	}

	seelog.Debugf("writeHeader step1")
	...
	seelog.Debugf("writeHeader step2")
